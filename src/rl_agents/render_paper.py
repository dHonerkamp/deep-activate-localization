...
class Trajectory:
    ...
    def render_step(observation, likelihood_map, gt_pose, est_pose, prev_gt_poses, prev_est_poses):
        ...
        axes[1].imshow(cv2.cvtColor((((1 - observation['likelihood_map'][..., 0])) * 255).astype(np.uint8), cv2.COLOR_GRAY2BGR))
        masked_data = np.ma.masked_where(observation['likelihood_map'][..., 1] == 0, observation['likelihood_map'][..., 1])
