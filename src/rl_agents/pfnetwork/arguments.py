...
def parse_common_args(env_name: str, collect_data: bool = False, add_rl_args: bool = False):
    ...
    argparser.add_argument('--num_train_samples', type=int, default=7500, help='Total number of samples to use for training. Total training samples will be num_train_samples=num_train_batches*batch_size')
    argparser.add_argument('--num_eval_samples', type=int, default=816, help='Total number of samples to use for evaluation. Total evaluation samples will be num_eval_samples=num_eval_batches*batch_size')
    argparser.add_argument('--num_test_samples', type=int, default=816, help='Total number of samples to use for testing. Total testing samples will be num_test_samples=num_test_batches*batch_size')

    argparser.add_argument('--num_particles', type=int, default=30, help='Number of particles in Particle Filter.')
    argparser.add_argument('--transition_std', nargs='*', default=DEFAULTS[env_name]['transition_std'], help='Standard deviations for transition model. Values: translation std (meters), rotation std (radians)')
    argparser.add_argument('--resample', type=str2bool, nargs='?', const=True, default=False, help='Resample particles in Particle Filter. Possible values: true / false.')
    ...
    argparser.add_argument('--init_particles_distr', type=str, default=DEFAULTS[env_name]['init_particles_distr'], help='Distribution of initial particles. Possible values: tracking / one-room.')
    argparser.add_argument('--init_particles_std', nargs='*', default=DEFAULTS[env_name]['init_particles_std'], help='Standard deviations for generated initial particles for tracking distribution. Values: translation std (meters), rotation std (radians)')

    argparser.add_argument('--likelihood_model', type=str, default='learned', choices=['learned', 'scan_correlation'])
    argparser.add_argument('--obs_mode', type=str, choices=["rgb", "depth", "rgb-depth", "occupancy_grid"], default='rgb-depth', help='Observation input type.')

    argparser.add_argument('--batch_size', type=int, default=DEFAULTS[env_name]['batch_size'], help='Minibatch size for training.')
    argparser.add_argument('--learning_rate', type=float, default=DEFAULTS[env_name]['batch_size'], help='Initial learning rate for training.')
    ...
    argparser.add_argument('--pfnet_loadpath', type=str, default='', help='Load a previously trained pfnet model from a checkpoint file.')

    argparser.add_argument('--global_map_size', nargs='*', default=DEFAULTS[env_name]['global_map_size'], help='Global map size in pixels (H, W, C)')
    argparser.add_argument('--map_pixel_in_meters', type=float, default=DEFAULTS[env_name]['map_pixel_in_meters'], help='The width (and height) of a pixel of the map in meters. Defaults to 0.02 for House3D data.')
    argparser.add_argument('--window_scaler', type=float, default=DEFAULTS[env_name]['window_scaler'], help='Rescale factor for extracing local map')
    ...
    if env_name == 'house3d':
        ...
        argparser.add_argument('--evalfiles', nargs='*', default=['/data2/honerkam/pfnet_data/valid.tfrecords'], help='Data file(s) for validation or evaluation (tfrecord).')
        argparser.add_argument('--testfiles', nargs='*', default=['/data2/honerkam/pfnet_data/test.tfrecords'], help='Data file(s) for testing (tfrecord).')
        ...
        # argparser.add_argument('--use_tf_function', type=str2bool, nargs='?', const=True, default=False, help="Whether to wrap pfnet model in tf.function.")
    ...
    if add_rl_args:
        argparser.add_argument('--num_iterations', type=int, default=1_000_000, help='Total number train/eval iterations to perform.')
        argparser.add_argument('--initial_collect_steps', type=int, default=1000, help='Number of steps to collect at the beginning of training using random policy')
        argparser.add_argument('--collect_steps_per_iteration', type=int, default=1, help='Number of steps to collect and be added to the replay buffer after every training iteration')
        argparser.add_argument('--train_steps_per_iteration', type=int, default=1, help='Number of training steps in every training iteration')

        argparser.add_argument('--replay_buffer_capacity', type=int, default=1_000_000, help='Replay buffer capacity per env.')
        argparser.add_argument('--rl_batch_size', type=int, default=256, help='Batch size for each training step. For each training iteration, we first collect collect_steps_per_iteration steps to the replay buffer. Then we sample batch_size steps from the replay buffer and train the model for train_steps_per_iteration times.')
        ...
        argparser.add_argument('--num_parallel_environments', type=int, default=1, help='Number of environments to run in parallel')
